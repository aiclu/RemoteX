...
- `{"type":"stage","stage":"download|verify|extract|helper-handoff|ready-to-swap|wait-exit|swap|swapped"}`
...
更新器参数保留旧的 URL/程序路径位置参数，并新增 `--pid`、`--target-version`、`--state-file` 和 `--restart`。暂存交接模式额外支持 `--target-pid` 和 `--ready-file`，同时保留 `--parent-pid` 兼容旧 bootstrap。`--pid` 优先于按进程名查找，同时继续兼容 `REMOTEX_TARGET_PID`。不可重定向 stdout 的 UAC `runas` 路径通过状态文件等待 `ready-to-swap` 或失败状态；用户取消 UAC 时旧版本保持运行。
...
正常交接必须先启动暂存更新器，并由它在完成暂存包和目标目录预检后写入一次性 `ready-file`。父更新器确认该握手、持久化暂存目录后，才发送 `ready-to-swap` 并允许 WPF 调用 `App.Close()`。暂存更新器使用独立标准句柄并尽力脱离父进程 Job；它随后用目标应用 PID和父更新器 PID共同等待，避免主程序退出时子进程或管道被一并终止。

更新日志至少包含阶段、进程 PID、时间戳、暂存路径、子进程启动结果和等待结果；`helper-handoff` 不是应用可关闭的成功信号，`ready-to-swap` 才是已完成交接的唯一信号。
